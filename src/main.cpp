...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // 1 days
unsigned int nStakeMaxAge = -1;           // unlimited
unsigned int nModifierInterval = 20 * 20; // time to elapse before new modifier is computed
...
const string strMessageMagic = "Zapphire Signed Message:\n";
...
static const int64_t nTargetTimespan =  20 * 20;  // 20 blocks
//
...
// zapphire: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "zapphire", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Zapphire 2017";
        CTransaction txNew;
        txNew.nTime = 1495989030;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1495989030;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1593143;
		if(fTestNet)
        {
            block.nNonce   = 1593143;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8623e8017f641832eba097464dd57283132862cdcb7a99ddab4631b71b44bfe6"));
        block.print();
